...
INSTALL_DIR="${INSTALL_DIR:-$HOME/.local/share/zoner}"
REPO_URL="${REPO_URL:-https://github.com/LoG1331/dns-dashboard/releases/download/v1.0.0/zoner-v1.0.0.tar.gz}"
BACKEND_PORT=5001
NODE_MAJOR=22
...
if command -v systemctl >/dev/null 2>&1 && systemctl --user is-system-running >/dev/null 2>&1; then
  ...
  cat > "$HOME/.config/systemd/user/zoner.service" <<EOF
[Unit]
...
EOF
  ...
  systemctl --user enable --now zoner
  ...
  setsid nohup node src/index.js >"$HOME/.local/share/zoner.log" 2>&1 &
fi
