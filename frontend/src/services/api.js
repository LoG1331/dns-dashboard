...
api.interceptors.response.use(
    ...
    (error) => {
        if (error.response?.status === 401) {
            // Check if not already on login to avoid loops
...
export const createZone = async (name, templateId = null) => {
    const response = await api.post('/zones', { name, template_id: templateId || undefined });
    return response.data;
};
